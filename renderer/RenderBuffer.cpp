...
void RenderBuffer::renderUsing(const WHBGfxShaderGroup *group, int binding) {
  GX2SetPixelTexture(&texture,
                     group->pixelShader->samplerVars[binding].location);
  GX2SetPixelSampler(&sampler,
                     group->pixelShader->samplerVars[binding].location);

  // Alpha off
  GX2SetBlendControl(GX2_RENDER_TARGET_0, GX2_BLEND_MODE_ONE,
                     GX2_BLEND_MODE_ZERO, GX2_BLEND_COMBINE_MODE_ADD, FALSE,
                     GX2_BLEND_MODE_ONE, GX2_BLEND_MODE_ZERO,
                     GX2_BLEND_COMBINE_MODE_ADD);
}

RenderBuffer::RenderBuffer(bool highPrecision, int width_override,
                           int height_override) {
  // Figure out sizes
  ...
  WHBLogPrintf("Setting up a render buffer with dimensions %i x %i", width,
               height);
  ...
  texture.surface.dim =
      GX2_SURFACE_DIM_TEXTURE_2D; // there's some fun ones like arrays or 3D
                                  // textures if we want those
  if (highPrecision) {
    texture.surface.format = GX2_SURFACE_FORMAT_FLOAT_R32_G32_B32_A32;
  } else {
    texture.surface.format = GX2_SURFACE_FORMAT_UNORM_R8_G8_B8_A8;
  }
  texture.surface.tileMode =
      GX2_TILE_MODE_DEFAULT; // probably memory layout stuff?
  texture.surface.aa =
      GX2_AA_MODE1X;         // anti-aliasing off (but we could set it to on?)
  ...
  texture.surface.image =
      memalign(texture.surface.alignment,
               texture.surface.imageSize); // TODO maybe mem1 needed
  GX2Invalidate(GX2_INVALIDATE_MODE_CPU_TEXTURE, texture.surface.image,
                texture.surface.imageSize);

  // Also set up a sampler
  GX2InitSampler(
      &sampler, GX2_TEX_CLAMP_MODE_CLAMP,
      GX2_TEX_XY_FILTER_MODE_LINEAR); // TODO can be point unless we do stupid
                                      // tricks for cheap blur in shader
  ...
  depthBuffer.surface.image =
      memalign(depthBuffer.surface.alignment, depthBuffer.surface.imageSize);
  GX2Invalidate(GX2_INVALIDATE_MODE_CPU_TEXTURE, depthBuffer.surface.image,
                depthBuffer.surface.imageSize);

  // Set up a context state (the object that holds buffer info, what you
  // actually bind to render to)
  contextState = (GX2ContextState *)memalign(GX2_CONTEXT_STATE_ALIGNMENT,
  ...
  GX2SetViewport(0, 0, (int)colorBuffer.surface.width,
                 (int)colorBuffer.surface.height, 0.0f, 1.0f);
  GX2SetScissor(0, 0, (int)colorBuffer.surface.width,
                (int)colorBuffer.surface.height);
