...
void getShaderCacheFileName(const char *filename, const char *source,
                            char *key) {
...
GX2VertexShader *loadVertexShaderFromCache(const char *filename,
                                           const char *source) {
  // Build the cache key
  char key[256];
  ...
  GX2VertexShader *shader =
      (GX2VertexShader *)memalign(64, sizeof(GX2VertexShader));
  fread(shader, sizeof(GX2VertexShader), 1, file);

  // Do a bit of setup
  shader->gx2rBuffer.flags =
      GX2R_RESOURCE_BIND_SHADER_PROGRAM | GX2R_RESOURCE_USAGE_CPU_READ |
  ...
  shader->uniformBlocks = (GX2UniformBlock *)memalign(
      64, shader->uniformBlockCount * sizeof(GX2UniformBlock));
  fread(shader->uniformBlocks, sizeof(GX2UniformBlock),
        shader->uniformBlockCount, file);

  // Read the loop vars
  shader->loopVars =
      (GX2LoopVar *)memalign(64, shader->loopVarCount * sizeof(GX2LoopVar));
  fread(shader->loopVars, sizeof(GX2LoopVar), shader->loopVarCount, file);

  // Read the sampler vars
  shader->samplerVars = (GX2SamplerVar *)memalign(
      64, shader->samplerVarCount * sizeof(GX2SamplerVar));
  fread(shader->samplerVars, sizeof(GX2SamplerVar), shader->samplerVarCount,
        file);

  // Read the attrib vars
  shader->attribVars = (GX2AttribVar *)memalign(64, shader->attribVarCount *
                                                        sizeof(GX2AttribVar));
...
// Check if the shader cache exists and load it if it does
GX2PixelShader *loadPixelShaderFromCache(const char *filename,
                                         const char *source) {
  // Build the cache key
  char key[256];
  getShaderCacheFileName(filename, source, key);
  ...
  GX2PixelShader *shader =
      (GX2PixelShader *)memalign(64, sizeof(GX2PixelShader));
...
bool saveVertexShaderToCache(const char *filename, const char *source,
                             GX2VertexShader *shader) {
  if (!shader || !filename) {
    return false;
  }
  ...
  // Build the cache key
  char key[256];
  getShaderCacheFileName(filename, source,
                         key); // Assuming a function to generate the filename
                               // from shader's metadata
  ...
  fwrite(shader->uniformBlocks, sizeof(GX2UniformBlock),
         shader->uniformBlockCount, file);
  ...
  fwrite(shader->samplerVars, sizeof(GX2SamplerVar), shader->samplerVarCount,
         file);

  // Write the attrib vars
  fwrite(shader->attribVars, sizeof(GX2AttribVar), shader->attribVarCount,
         file);
...
bool savePixelShaderToCache(const char *filename, const char *source,
                            GX2PixelShader *shader) {
  if (!shader || !filename) {
    return false;
  }
...
WHBGfxShaderGroup *compileOrLoadShader(const char *vsPath, const char *psPath,
                                       const char *vsSrc, const char *psSrc) {
  char infoLog[1024];
...
#ifndef CACHE_ONLY
    maybeInitGLSL();
    vs = GLSL_CompileVertexShader(vsSrc, infoLog, sizeof(infoLog),
                                  GLSL_COMPILER_FLAG_NONE);
#endif
    if (!vs) {
      WHBLogPrintf(
          "Failed to either load or compile vertex shader. Infolog: %s\n",
          infoLog);
      return NULL;
    }
...
#ifndef CACHE_ONLY
    maybeInitGLSL();
    ps = GLSL_CompilePixelShader(psSrc, infoLog, sizeof(infoLog),
                                 GLSL_COMPILER_FLAG_NONE);
#endif
    if (!ps) {
...
ShaderManager::ShaderManager() {
  ...
  attribs.push_back(AttribSpec{"in_texcoord", BufferType::COLOR,
...
WHBGfxShaderGroup *
ShaderManager::shader(const char *vsPath, const char *psPath,
  ...
  WHBGfxShaderGroup *group =
      compileOrLoadShader(vsPath, psPath, vsCode, psCode);
  if (!group) {
  ...
  for (uint32_t i = 0; i < attribs.size(); ++i) {
    WHBLogPrintf("Attaching attributes %s - %i @ %d", attribs[i].name.c_str(),
                 attribs[i].format, i);
    WHBGfxInitShaderAttribute(group, attribs[i].name.c_str(), i, 0,
                              attribs[i].format);
  }
