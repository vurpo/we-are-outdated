...
MusicPlayer::MusicPlayer(const char *oggFileName, float offsetSeconds) {
  AXInitParams init = {AX_INIT_RENDERER_48KHZ, 0,
                       0}; // possible sample rates are 48k and 32k, rest of
  ...
  if (ov_open_callbacks(audioFile, &vorbisDecoderState, NULL, 0,
  ...
  int16_t pcmout[2048]; // Temporary buffer for decoded data, half the size
                        // since int16_t is 2 bytes
  int section = 0;
  int offsetSamples = offsetSeconds * sampleRate * 2.0;
  WHBLogPrintf("Offsetting %d samples", offsetSamples);
  do {
    readSize = ov_read(&vorbisDecoderState, (char *)pcmout, sizeof(pcmout), 1,
                       2, 1, &section);
  ...
  WHBLogPrintf("Loaded data: %d samples at rate %d", decodedData.size() / 2,
               sampleRate);
  ...
  // Init the playback
  voiceLeft = AXAcquireVoice(31,   // Priority, higher = more important. doesn't
                                   // matter for us. 31 is max, in any case.
                             NULL, // Status callback. We don't care about this.
                             NULL  // User data
  );
  voiceRight = AXAcquireVoice(31, NULL, NULL);

  // Lets go Full Crime: no callback just dump the entire buffer at the DSP in
  // one go I *think* what you would do with the callback is have the buffer
  // loop and then fill in the audio callback if you go past halfway / past end,
  // pingponging between halves (and make the buffer 6ms since audio callback)
  ...
  AXSetVoiceSrcType(voiceLeft,
                    AX_VOICE_SRC_TYPE_NONE); // we do not want to pitch bend
  AXSetVoiceSrcRatio(voiceLeft, 1.0f);       // ignored
  AXSetVoiceSrcType(voiceRight,
                    AX_VOICE_SRC_TYPE_NONE); // we do not want to pitch bend
  AXSetVoiceSrcRatio(voiceRight, 1.0f);      // ignored

  // Set up volume
  AXVoiceVeData volumeInfo = {
      0x8000, // Volume actual. These are fixedpoint 1.15 values, so 0x8000
              // is 1.0
  ...
  // Set up mixing
  // There are (for TV) six channels, with four "buses", this structure contains
  // volume / delta for each
  ...
  AXSetVoiceDeviceMix(voiceLeft, AX_DEVICE_TYPE_TV, 0,
                      (AXVoiceDeviceMixData *)&deviceVolumeData);
  AXSetVoiceDeviceMix(
      voiceLeft, AX_DEVICE_TYPE_DRC, 0,
      (AXVoiceDeviceMixData *)&deviceVolumeData); // <-- uncomment for gamepad
  deviceVolumeData[0].bus[0].volume = 0;          // Left channel
  deviceVolumeData[1].bus[0].volume = 0x8000;     // Right channel
  AXSetVoiceDeviceMix(voiceRight, AX_DEVICE_TYPE_TV, 0,
                      (AXVoiceDeviceMixData *)&deviceVolumeData);
  AXSetVoiceDeviceMix(
      voiceRight, AX_DEVICE_TYPE_DRC, 0,
      (AXVoiceDeviceMixData *)deviceVolumeData); // <-- uncomment for gamepad

  // Wiimotes (TODO, WUT doesn't define these)
  // AXSetVoiceDeviceMix(voice, AX_DEVICE_TYPE_RMT, 0, 0, &deviceVolumeData); //
  // <-- uncomment for wiimote 0 AXSetVoiceDeviceMix(voice, AX_DEVICE_TYPE_RMT,
  // 0, 1, &deviceVolumeData); // <-- uncomment for wiimote 1
...
void MusicPlayer::seek(float seconds) {
  ...
  bufferInfo.currentOffset =
      std::min((uint32_t)(seconds * sampleRate), bufferInfo.endOffset - 1);
  AXSetVoiceOffsets(voiceLeft, &bufferInfo);
